...
// Rate limiter for sensitive unauthenticated endpoints
...
// Safe user fields — never expose internal DB IDs or timestamps
const SAFE_USER_FIELDS = ['walletAddress', 'chainId', 'status', 'approvalStatus', 'approvalTxHash'];

function sanitizeUser(user) {
    if (!user) return null;
    const raw = user.toJSON ? user.toJSON() : user;
    const safe = {};
    SAFE_USER_FIELDS.forEach(f => { if (raw[f] !== undefined) safe[f] = raw[f]; });
    return safe;
}
 ...
 */
router.post('/approve', approveRateLimiter, [
    ...
    try {
        ...
        // Prevent tx hash reuse — each approval tx should be unique
        const existingTx = await User.findOne({ where: { approvalTxHash } });
        if (existingTx && existingTx.walletAddress !== walletAddress.toLowerCase()) {
            return res.status(409).json({
                success: false,
                message: 'Transaction hash already used by another wallet.'
            });
        }
