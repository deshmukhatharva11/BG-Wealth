...
app.use(cors({
    origin: function (origin, callback) {
        ...
        // Cloudflare Tunnel origins must be explicitly listed in ALLOWED_ORIGINS
...
const adminLimiter = rateLimit({
    windowMs: 1 * 60 * 1000,  // 1 minute window
    max: 500,                  // 500 req/min — admin polling + bulk ops
...
// BG-Wealth frontend assets
app.get('/bg-wealth-styles.css', (req, res) => {
    res.set('Cache-Control', 'no-cache, no-store, must-revalidate');
    res.sendFile(path.join(projectRoot, 'bg-wealth-styles.css'));
});
app.get('/bg-wealth-app.js', (req, res) => {
    res.set('Cache-Control', 'no-cache, no-store, must-revalidate');
    res.sendFile(path.join(projectRoot, 'bg-wealth-app.js'));
});
app.get('/particle-bg.js', (req, res) => {
    res.set('Cache-Control', 'public, max-age=3600');
    res.sendFile(path.join(projectRoot, 'particle-bg.js'));
});
app.get('/BGWealth.jpg', (req, res) => {
    res.set('Cache-Control', 'public, max-age=3600');
    res.sendFile(path.join(projectRoot, 'BGWealth.jpg'));
});
