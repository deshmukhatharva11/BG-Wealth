...
async function fetchBatchWithRetry(batch, fetcherAddr, batchNum) {
    ...
    // All retries exhausted — throw to skip this batch (preserve existing DB data)
    throw new Error(`Batch ${batchNum} failed after ${MAX_RETRIES} retries: ${lastError?.message || 'unknown'}`);
}
